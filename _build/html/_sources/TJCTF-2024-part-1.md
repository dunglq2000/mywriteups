...
File `log.txt` khá dài nên mình lưu trong folder chứa project writeup này `TJCTF/2024/assume`. Ở bài này mình chỉ cần kiểm tra xem if hay else được thực thi bằng việc kiểm tra message thứ 5 của mỗi đoạn có bằng `(g^a)^b` không.
...
Với hint là file `keysmashes.txt` (mình cũng lưu trong repo này ở `TJCTF/2024/hulksmash`) thì các dòng trong file sẽ giúp mình tìm key.

Để ý rằng mỗi dòng chỉ gồm các ký tự `fjdlska;` (8 ký tự, mỗi ký tự xuất hiện 2 lần).

Để ý thêm xíu nữa :))) mình phát hiện ra mỗi ký tự xuất hiện 2 lần vào vị trí hoặc đều chẵn, hoặc đều lẻ. Cụ thể là các ký tự `fdsa` sẽ ở vị trí chẵn còn `jlk;` ở vị trí lẻ.
